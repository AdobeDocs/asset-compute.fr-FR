...
| `target` | `object` | Informations de chargement d’URL présignée en plusieurs parties pour le rendu généré. Ces informations concernent le [chargement binaire direct AEM/Oak](https://jackrabbit.apache.org/oak/docs/features/direct-binary-access.html) avec ce [comportement de chargement multipartie](https://jackrabbit.apache.org/oak/docs/apidocs/org/apache/jackrabbit/api/binary/BinaryUpload.html).<br>champs :<ul><li>`urls` : tableau de chaînes, une pour chaque URL de partie pré-signée</li><li>`minPartSize` : taille minimale à utiliser pour une partie = url</li><li>`maxPartSize` : taille maximale à utiliser pour une partie = url</li></ul> | `{ "urls": [ "https://part1...", "https://part2..." ], "minPartSize": 10000, "maxPartSize": 100000 }` |
...
| `dpi` | `number` ou `object` | Définir la résolution en ppp pour x et y. Pour plus de simplicité, il est également possible de définir un nombre unique appliqué à la fois à x et y. Cela n’a aucun effet sur l’image elle-même. | `96` ou `{ xdpi: 96, ydpi: 96 }` |
...
| Nom | Type | Description | Exemple |
|-------------------|----------|-------------|---------|
| `scale` | `number` | Échelle du filigrane, entre `0.0` et `1.0`. `1.0` signifie que le filigrane a son échelle d’origine (1:1) et que les valeurs les plus faibles réduisent la taille du filigrane. | Une valeur de `0.5` correspond à la moitié de la taille d’origine. |
